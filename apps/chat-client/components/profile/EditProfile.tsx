...
const EditProfile = ({
  ...
  if (!user) {
    return (
      <div className="h-[50vh] flex-1 grid place-items-center text-gray-500 text-2xl italic">
        <p>
          Error in getting user details, Try{" "}
          <Link href={"/auth/signin"} className="underline hover:text-gray-400">
            Signing in
          </Link>{" "}
          again.
        </p>
  ...
  const toBase64: (file: Blob) => Promise<string | ArrayBuffer | null> = async (
    file: Blob
  ) => {
      ...
      <form onSubmit={handleSubmit}>
        ...
        <fieldset className="grid grid-cols-3 border border-gray-600 bg-gray-800 rounded-2xl px-5 py-4 gap-5 mb-5">
          ...
          <div className="col-span-2 flex flex-col justify-center">
            ...
            />
            <p className="mt-2 px-3 text-xs text-gray-500 italic font-mono">
              only .png .jpg .jpeg .webp (under 800Kb)
            </p>
          </div>
        </fieldset>
        <div className="flex items-center">
          ...
          <p className="text-xs text-gray-500 italic">
            You will Need to Sign In again after update...
          </p>
        </div>
      </form>
      <div>
